...
void vrng_shadow_complete(struct vrng_shadow *shadow, u32 produced)
{
	...
	if (!shadow->active)
		goto unlock;
	c_result = vrng_core_c_complete(&shadow->c_state, shadow->generation,
					produced, &c_resubmit);
#if IS_ENABLED(CONFIG_HW_RANDOM_VIRTIO_LANG_RUST)
	rust_result = vrng_core_rust_complete(&shadow->rust_state,
					      shadow->generation, produced,
					      &rust_resubmit);
#else
	rust_result = c_result;
		      ...
		      u32 requested)
{
	...
	bytes_differ |= c_copied != rust_copied ||
		memcmp(c_buffer, rust_buffer, sizeof(c_buffer));
#else
	rust_result = c_result;
	...
	bytes_differ |= c_copied != mc_copied ||
		memcmp(c_buffer, mc_buffer, sizeof(c_buffer));
#else
	mc_result = c_result;
	mc_copied = c_copied;
	mc_resubmit = c_resubmit;
